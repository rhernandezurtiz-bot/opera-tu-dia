...
import { Home, FilePlus2, ListOrdered, ChefHat, AlertTriangle, Settings } from "lucide-react";
import type { ReactNode } from "react";

const nav = [
  { to: "/", label: "Inicio", icon: Home },
  { to: "/nuevo", label: "Nuevo pedido", icon: FilePlus2 },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <main className="flex-1 min-w-0 pb-24 md:pb-8">
        {/* Mobile header */}
        <header className="md:hidden sticky top-0 z-30 bg-background/85 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-2">
          <div className="h-8 w-8 rounded-lg bg-primary text-primary-foreground grid place-items-center font-display">O</div>
          <div className="font-display text-lg">Operia</div>
        </header>
        <div className="px-4 md:px-10 py-6 md:py-10 max-w-6xl mx-auto">{children}</div>
      </main>

      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-card/95 backdrop-blur border-t border-border px-2 py-2 grid grid-cols-6 gap-1">
        {nav.map((n) => {
          const active = n.to === "/" ? loc.pathname === "/" : loc.pathname.startsWith(n.to);
          const Icon = n.icon;
          return (
